...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, Raccoon does not support MCP authentication yet.` // raccoon_change - rebrand user-facing error
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`raccoon models\` to list available models`, // raccoon_change - rebrand suggested command
      `Or check your config (raccoon.json) provider/model names`, // raccoon_change - prefer raccoon config name
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`raccoon auth login ${url}\` to re-authenticate.`] : []), // raccoon_change - rebrand suggested command
    ].join("\n")
  }
