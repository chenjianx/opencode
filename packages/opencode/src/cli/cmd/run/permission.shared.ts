...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Raccoon is restarted.`] // raccoon_change - rebrand permission lifetime copy
  }

  return [
    "This will allow the following patterns until Raccoon is restarted.", // raccoon_change - rebrand permission lifetime copy
