...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      // raccoon_change start - resolve theme dir under .raccoon, accept legacy .opencode
      path.basename(source_dir) === ".raccoon" || path.basename(source_dir) === ".opencode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".raccoon", "themes")
    // raccoon_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".raccoon", "tui.json"), // raccoon_change - default tui config under .raccoon
  }
}
