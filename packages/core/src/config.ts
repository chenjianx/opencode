...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "opencode.json", "opencode.jsonc", "raccoon.json", "raccoon.jsonc"] // raccoon_change - accept raccoon config file names
    ...
    // raccoon_change start - discover both `.raccoon` and legacy `.opencode` config dirs
    const configDirNames = [".raccoon", ".opencode"]
    const isConfigDir = (item: string) => configDirNames.includes(path.basename(item))
    // raccoon_change end
    const discovered = locationIsGlobal
      ? []
      : yield* fs
          .up({
            targets: [...configDirNames, ...names.toReversed()], // raccoon_change - include raccoon config dirs
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter(isConfigDir) // raccoon_change - match both raccoon and opencode config dirs
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => !isConfigDir(item)).toReversed() // raccoon_change - exclude both raccoon and opencode config dirs
    ...
    // global config, project files, then `.raccoon`/`.opencode` files. // raccoon_change - mention raccoon dirs
